...
def test_format_ota_detail_includes_stored_timestamp_and_cached_run_metadata() -> None:
    row = OtaFailureRow(
        last_run=456,
        last_modified="2024-09-05T12:34:56+00:00",
    ...
    assert "last_modified: 2024-09-05 12:34Z" in detail
    assert "run_updated_at: 2024-09-03 12:34Z" in detail
    assert "run_title: Extract OTA symbols" in detail


def test_format_ota_detail_prefers_stored_last_modified() -> None:
    row = OtaFailureRow(
        last_run=456,
        last_modified="2024-09-05T12:34:56+00:00",
        processing_state=ArtifactProcessingState.INDEXED_INVALID,
    ...
    detail = format_ota_detail(row, None)
