...
This is a trade-off, not an obvious replacement. The current JSON model is awkward and makes metadata writes high-stakes, but it provides cheap full-domain aggregate snapshots. The object-per-artifact v2 model should only win if reduced mutation contention/blast radius and source extensibility are worth the increased object count, aggregate-read, cache, and model complexity. A valid positive outcome of this evaluation is a decision document that explains the measured trade-offs, complexity effects, why we should not migrate to this model, and what smaller alternatives we should pursue instead. See [`015-cost-accounting-and-tradeoff.md`](015-cost-accounting-and-tradeoff.md).
